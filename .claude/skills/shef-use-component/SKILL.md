...
**Как выбрать между вариантами — по тому, что вы делаете с файлами, а не по
оценке «можно ли вынести».** Правите `class.php` компонента в этой задаче
(добавляете метод, меняете `process()`, что угодно) — берите вариант 1: вы
уже внутри файла, вынести логику в `lib/` стоит ровно столько же. Компонент
существует, и вы его **не трогаете** — тогда вариант 2.

Формулировка «если вынести нельзя» не работает: на прогоне решатель правил
`class.php`, добавляя публичный метод, и всё равно шёл вторым вариантом —
слова «сервис» в диалоге из 22 действий не было ни разу. Оценочное условие
агент пропускает, проверяемое — нет.
...
Употреблений два, и параметры доходят только до одного из них.
...
### Живой объект компонента, без выполнения

Это и есть ответ на «получить данные компонента без HTML»:
...
только через `include()` и `includeSlider()`.

Вместе с параметрами не выполняется **весь** жизненный цикл. Перечислить
стоит поимённо, иначе «не выполняет `executeComponent()`» ни о чём не говорит
тому, кто конвейер наизусть не помнит:

```
initErrorCollection → initAutoloader → includeModules → initParams
→ checkRequiredParams → setPageProperty → initResult → process → renderTemplate
└── это делает buildClass() ──┘   └──────── этого не будет ────────┘
```
