...
description: Набор трейтов модуля shef.options для Битрикс24 — подключение модулей (Modules), сбор ошибок (ErrorCollection), защита обработчиков событий от повторного вызова (Events), приведение входных полей (PrepareFields), работа с датой, кодировкой, XmlId, режимом отладки, константами каталога, цен, сайта и пользователя. Брать, когда в классе модуля линейки shef.* нужна одна из этих механик и не хочется писать её заново.
---
