...
description: Создать новый модуль линейки shef.* для Битрикс24 «коробки» или БУС по канону — раскладка файлов, install/index.php с CModule, .settings.php, include.php и autoload.php, страница настроек, языковые файлы. Вызывать, когда нужен новый локальный модуль, который опирается на shef.options, или когда надо проверить, всё ли на месте в уже заведённом.
---
...
* `InstallFiles()` / `UnInstallFiles()` по карте `installDir`;
