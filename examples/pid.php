...
// Читается как вывод о стенде, хотя это правило: на портале с работающим
// /proc текст говорил бы обратное тому, что видно выше. Поэтому правило
// отдельно, а факты об окружении — отдельно, и без пересказа решения
// isProcessAlive(): печатаем те же два признака, по которым решает она.
note('Правило: когда выяснить, жив ли процесс, нечем, файл ОСТАЁТСЯ. Лишняя');
